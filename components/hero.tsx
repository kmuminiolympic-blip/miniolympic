...
import { Calendar, MapPin, Users, Heart, Newspaper, Info } from "lucide-react"

// 산뜻한 에메랄드 민트 색상 정의 (Tailwind 클래스 사용)
const pointColorText = "text-emerald-600" // 주 포인트 텍스트 및 아이콘용
const pointColorBg = "bg-emerald-100" // 연한 배경색용
const pointColorHover = "group-hover:text-emerald-700" // 호버 시 약간 짙어지는 색

export function Hero() {
  const mainMenus = [
    { label: "행사소개", href: "#about", icon: Info },
    { label: "참가부스", href: "/booth", icon: Users }, // ← 여기 수정
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-3 items-start pt-12 lg:pt-20 gap-8">
          ...
          <div className="flex flex-col items-center lg:items-end gap-3 text-right">
            <div className="flex items-center gap-2 text-foreground/80">
              ...
              <span className="text-sm lg:text-base font-bold">2026. 05. 27 (수) 09:00~18:00</span>
            </div>
            <div className="flex items-center gap-2 text-foreground/80">
              <MapPin className={`w-4 h-4 ${pointColorText}`} />
              <span className="text-sm lg:text-base font-bold text-muted-foreground">
                계명대학교 구바우어관 풋살장 / 농구장
              </span>
            </div>
          </div>
